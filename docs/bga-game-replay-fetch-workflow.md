...
Для автоматичного запиту `request_class` дорівнює `queue_class`; прямий запуск команди для однієї гри або матчу записується як `manual`.
...
5. Створити таблицю `bga_replay_requests` із `request_class` та індекси для підрахунку загального й історичного рухомого 24-годинного бюджету.
6. Визначити безпечні значення для наявних записів:
...
6. Додати ідемпотентну внутрішню операцію enqueue для історичної гри без прив'язки до майбутнього UI або способу позначення.
7. Перед кожним історичним запитом перевіряти відсутність due-записів `fresh`.
8. Для історичного режиму обмежити один запуск трьома запитами — максимум по одному на доступний акаунт.
...
18. Історичні запити не перевищують 30 на акаунт за рухомі 24 години і сумарно не можуть використати `FRESH_RESERVE`.
19. Якщо свіжі запити вже використали частину незахищеного бюджету, допустима кількість історичних запитів відповідно зменшується.
...
- історичний трафік обмежений 30 запитами на акаунт за рухомі 24 години та не може використати захищену місткість для свіжих ігор;
- максимальний історичний трафік становить 90 запитів за рухомі 24 години для трьох акаунтів і автоматично зменшується за наявності свіжого або ручного трафіку;

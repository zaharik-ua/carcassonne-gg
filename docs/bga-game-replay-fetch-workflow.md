...
- `--queue-class historical` — раз на 2 хвилини, до трьох `logs.html` за запуск і не більше одного запиту на кожен доступний акаунт. Це дозволяє вчасно перевіряти щойно замовлені архіви; rolling budget як і раніше обмежує загальний трафік.
...
Рекомендовані systemd timers можуть запускати свіжу та історичну смуги раз на 2 хвилини. Для historical це потрібно, щоб вчасно повторно перевірити щойно замовлений архів. Частота запуску не визначає кількість BGA-запитів: її обмежують клас черги, `next_attempt_at`, per-run limit та персистентний бюджет. Due-перевірки вже замовлених архівів мають пріоритет над ще не початим historical backlog.
...
3. Запускати `historical` окремим timer раз на 2 хвилини, щоб витримувати коротке вікно повторної перевірки замовленого архіву.

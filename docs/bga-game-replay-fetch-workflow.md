...
- дати глобальному адміністратору огляд використання трьох акаунтів і контрольоване тимчасове розширення historical-пулу.
...
CREATE TABLE bga_replay_budget_overrides (
  id INTEGER PRIMARY KEY,
  account_label TEXT NOT NULL,
  extra_historical_limit INTEGER NOT NULL,
...
### 9.2. Тимчасове розширення historical-пулу

Глобальний адміністратор може тимчасово передати частину захищеного fresh-резерву історичній черзі. Рекомендована готова дія: `+20 historical` для кожного з трьох акаунтів.

Для активного розширення `E`, де `0 <= E <= 20`, ефективні межі одного акаунта стають:

```text
effective_historical_limit = HISTORICAL_LIMIT + E
effective_fresh_reserve = FRESH_RESERVE - E
TOTAL_LIMIT = 80
```

При `E = 20` historical-пул зростає з 30 до 50, а захищений fresh-резерв зменшується з 50 до 30. Загальний ліміт 80 не збільшується. Для трьох акаунтів теоретична historical-місткість активного вікна зростає з 90 до 150 запитів, якщо інші класи ще не використали відповідну місткість.

Під час активного розширення historical-запит дозволено, коли:

```text
historical_used_24h < effective_historical_limit
total_used_24h < TOTAL_LIMIT - effective_fresh_reserve
```

Тому `+20` означає збільшення виділеного historical-пулу на 20, але фактичний залишок може бути меншим, якщо частину загального rolling-бюджету вже використали `fresh` або `manual`. Admin UI має показувати не лише номінальні `30 + 20`, а й обчислене поле `historical_available_now`.

Вимоги до розширення:

- воно створюється окремо для кожного акаунта; дія «+20 для всіх» атомарно створює три записи;
- повторне натискання не накопичує `+40`: на акаунт може діяти лише одне розширення, максимум `+20`;
- адміністратор обов'язково задає `expires_at`; рекомендоване максимальне вікно — 12 годин;
- розширення можна відкликати раніше через `revoked_at`;
- причина, автор і час зберігаються для аудиту;
- worker перевіряє актуальність розширення перед кожним historical-запитом;
- після завершення або відкликання нові historical-запити знову підкоряються базовому ліміту 30;
- вже виконані запити не скидаються й залишаються у rolling-вікні 24 години;
- `fresh` завжди зберігає пріоритет над `historical`, навіть коли розширення активне;
- `budget_override_id` записується для запитів, дозволених активним розширенням, щоб показувати фактичне використання додаткового пулу.

Цей control не повинен дозволяти підвищувати `TOTAL_LIMIT` понад 80. Якщо колись знадобиться збільшення саме загального ліміту, це має бути окремим рішенням із додатковим підтвердженням, а не побічним ефектом historical boost.
...
- загалом використано з `TOTAL_LIMIT = 80`;
- базовий historical limit, активне розширення та час його завершення;
...
- `Give +20 historical to all accounts` із вибором `expires_at` і обов'язковою причиною;
- таке саме розширення для одного вибраного акаунта;
- дострокове відкликання активного розширення;
- refresh стану без запуску BGA-запитів.

Перед підтвердженням `+20` діалог показує для кожного акаунта значення до і після: historical limit, `historical_available_now`, fresh reserve, загальне використання та cooldown. Якщо є due/scheduled fresh-ігри, діалог показує попередження, але остаточне рішення залишається за глобальним адміністратором.

Мінімальний внутрішній API:

```text
GET    /admin/bga-replay-budget
POST   /admin/bga-replay-budget/historical-boost
DELETE /admin/bga-replay-budget/historical-boost/{id}
```

`POST` приймає `account_labels`, `extra_historical_limit`, `expires_at` і `reason`. Сервер сам обмежує значення максимумом `+20`, перевіряє global-admin access і створює записи в одній транзакції. Клієнт не передає обчислені залишки бюджету.
...
6. Створити `bga_replay_budget_overrides` для персистентних, обмежених у часі historical-розширень.
...
9. Додати до gateway активне розширення до `+20` historical-запитів без збільшення `TOTAL_LIMIT`.
10. Заборонити накопичення кількох одночасних розширень для одного акаунта.
...
### Етап 8. Додати admin monitoring і historical boost
...
5. Додати створення `+20 historical` для одного або всіх акаунтів із `expires_at` і причиною.
6. Додати відкликання розширення та повний audit trail.
7. Перед підтвердженням показувати фактично доступний додатковий пул і попередження про due/scheduled fresh-ігри.
...
24. `+20` збільшує historical limit з 30 до 50 і зменшує захищений fresh reserve з 50 до 30, не змінюючи `TOTAL_LIMIT = 80`.
25. Повторне створення розширення не накопичує понад `+20` для одного акаунта.
26. Прострочене або відкликане розширення не дозволяє нових запитів понад базовий historical limit.
27. Запити, виконані через розширення, мають `budget_override_id` і залишаються в rolling-вікні після його завершення.
28. Навіть з активним розширенням fresh due-запис обробляється раніше за historical.
...
14. Після перевірки аудиту, expiry та fresh-пріоритету дозволити `+20` для всіх трьох акаунтів.
...
- базовий максимальний historical-пул становить 90 запитів за рухомі 24 години для трьох акаунтів; активне `+20` для кожного акаунта тимчасово піднімає його до 150, не збільшуючи загальний per-account limit;
...
- global admin може тимчасово додати до `+20` historical-спроб на акаунт із expiry, audit trail і можливістю відкликання;
- historical boost не збільшує загальний ліміт 80 і не змінює абсолютний пріоритет fresh-черги;

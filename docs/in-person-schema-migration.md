...
Скидання помилкового результату не потребує окремої schema migration або таблиці
історії: воно очищає наявні result-поля `in_person_matches`, збільшує `revision`
матчу та за потреби повертає його раунд із `completed` у `published`.

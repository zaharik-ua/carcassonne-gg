...
Скидання помилкового результату є service-layer recovery-операцією, а не новим
result mode pure engine. Вона повертає звичайний матч у `scheduled` та очищає
поточні result fields. Пара, номер столу й `starting_participant_id` зберігаються;
system bye не скидається. Для плейоф service також атомарно очищає успадковані
слоти незавершених залежних матчів і блокує операцію, якщо залежний матч уже
завершений.

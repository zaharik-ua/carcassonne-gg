...
def _cache_path() -> Path:
    raw_path = os.getenv("BGA_PLAYER_ID_CACHE_FILE", ".cache/player_id_cache.json")
    path = Path(raw_path)
...
def get_player_id(nickname: str) -> int:
    ...
    if cache_key in cache:
        ...
        if current_time - cached["timestamp"] < CACHE_DURATION:
            print(f"✅ Using cached ID {cached['id']} for {nickname}", flush=True)
            return int(cached["id"])

    print(f"🌐 Fetching ID for {nickname} from BGA...", flush=True)
    ...
    for user in data.get("items", []):
        if user.get("q", "").lower() == nickname:
            ...
            save_cache(cache)
            print(f"✅ Found ID {player_id} for {nickname}", flush=True)
            return player_id

...
- Accounts 1–3 form the normal shared replay-account pool. Account 4 is the
  first standby and can be selected while every configured account 1–3 either
  has an active cooldown or has zero total requests available in its rolling
  budget. Account 5 is the second standby and uses the same condition across
  configured accounts 1–4.

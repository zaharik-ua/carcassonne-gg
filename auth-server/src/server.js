...
function normalizeLocalWebsiteName(value) {
...
async function loadLocalWebsiteById(localWebsiteId) {
  ...
  return dbGetAsync(
    `
      SELECT
        lw.id,
        COALESCE(NULLIF(trim(lw.name), ''), NULLIF(trim(lw.url), '')) AS name,
        COALESCE(NULLIF(trim(lw.name), ''), NULLIF(trim(lw.url), '')) AS url,
        lw.country_id,
        lw.created_by,
        lw.updated_by,
        lw.created_at,
...
async function getNewsEditorAccessForUser(user) {
...
async function countLocalWebsiteReferences(localWebsiteId) {
  const normalizedId = normalizePositiveInteger(localWebsiteId);
  if (!normalizedId) return { icons: 0, news: 0, news_editors: 0, total: 0 };
  const row = await dbGetAsync(
    `
      SELECT
        (SELECT COUNT(*) FROM icons WHERE local_website_id = ?) AS icons,
...
function isValidTeamType(value) {
  const normalized = String(value ?? "").trim().toLowerCase();
    ...
    `
      SELECT
        ...
        CASE
          ...
          ELSE 0
        END AS access_via_access_users,
        (
          SELECT GROUP_CONCAT(tt.team_id, '|')
          FROM tournament_teams tt
          WHERE upper(trim(tt.tournament_id)) = upper(trim(t.id))
            AND trim(tt.captain_id) = trim(?)
        ) AS captain_team_ids_csv
      FROM tournaments t
...
function normalizeTournamentCaptainTeamIds(value) {
  const entries = Array.isArray(value)
    ? value
    : String(value || "").split("|");
  return Array.from(new Set(
    entries
      .map((teamId) => String(teamId || "").trim().toUpperCase())
      .filter(Boolean)
  ));
}
...
function canClosedTournamentCaptainAccessMatch(tournament, userAssociation, team1, team2) {
  ...
  if (captainTeamIds.some((teamId) => teamId === normalizedTeam1 || teamId === normalizedTeam2)) {
    return true;
  }
  if (tournament?.access_via_access_users !== true) return false;
  const association = String(userAssociation || "").trim().toUpperCase();
  return !!association && (association === normalizedTeam1 || association === normalizedTeam2);
}
...
async function resolveTournamentTeamReferences(tournamentId, teamId, captainId, options = {}) {
  ...
  const [tournament, team, captain] = await Promise.all([
    dbGetAsync(
      `
        SELECT id, tournament_type, team_type
        FROM tournaments
        WHERE upper(trim(id)) = upper(trim(?))
        LIMIT 1
      `,
  ...
  const selectedTeamType = normalizeTeamType(team.type);
  if (selectedTeamType !== tournamentTeamType) {
    throw createTournamentTeamRequestError(
      400,
      `team_id must reference a ${tournamentTeamType} team`
    );
  }
  if (
    captain
    ...
    throw createTournamentTeamRequestError(
      400,
      "For National tournaments, captain association must match team_id"
    );
  }
    ...
    CREATE TABLE IF NOT EXISTS duels (
      ...
      CHECK (
        ...
        OR (
          ...
          AND status IN ('Draft', 'Requested new time', 'Planned', 'In progress', 'Done', 'Error', 'Cancelled')
        )
      )
    )
  `, (createErr) => {
...
function ensureChallengesSchema() {
  ...
  `, (createErr) => {
    ...
    ensureChallengePeriodPlayersSchema(db).catch((migrationError) => {
      console.error("Failed to migrate challenge_period_players schema", migrationError);
    });
  });
...
function ensureLocalWebsitesSchema() {
  ...
  `, (createErr) => {
    ...
    db.all("PRAGMA table_info(local_websites)", (pragmaErr, columns) => {
      ...
      db.serialize(() => {
        addColumnIfMissing(columns, "local_websites", "name", "TEXT");
        ...
        db.run(
          `
            ...
            SET
              name = COALESCE(NULLIF(trim(name), ''), NULLIF(trim(url), '')),
              url = COALESCE(NULLIF(trim(url), ''), NULLIF(trim(name), '')),
        ...
        db.run("DROP INDEX IF EXISTS idx_local_websites_url_unique", (dropErr) => {
          if (dropErr) {
            console.error("Failed to drop old unique index for local_websites.url", dropErr);
          }
        });
        db.run(
          "CREATE UNIQUE INDEX IF NOT EXISTS idx_local_websites_name_unique ON local_websites(lower(trim(name)))",
          (indexErr) => {
            if (indexErr) {
              console.error("Failed to ensure unique index for local_websites.name", indexErr);
            }
          }
        );
        db.run(
...
function ensureIconsSchema() {
  ...
  `, (createErr) => {
    ...
    db.all("PRAGMA table_info(icons)", (pragmaErr, columns) => {
      ...
      addColumnIfMissing(columns, "icons", "local_website_id", "INTEGER");
      ...
      db.run(
        "CREATE INDEX IF NOT EXISTS idx_icons_local_website_id ON icons(local_website_id)",
        (indexErr) => {
          if (indexErr) {
            console.error("Failed to ensure index for icons.local_website_id", indexErr);
          }
        }
      );
      db.run(
...
app.get("/local-websites", (_req, res, next) => {
  db.all(
    `
      ...
      LEFT JOIN associations a
        ON upper(trim(COALESCE(a.code, ''))) = upper(trim(COALESCE(lw.country_id, '')))
      WHERE trim(COALESCE(lw.name, lw.url, '')) <> ''
      ORDER BY lower(COALESCE(a.name, lw.country_id, '')) ASC, lower(COALESCE(lw.name, lw.url, '')) ASC, lw.id ASC
    `,
    (err, rows) => {
      if (err) return next(err);
...
app.post("/local-websites", requireAdmin, async (req, res) => {
  try {
    const actorPlayerId = String(req.user?.player_id || "").trim() || null;
    const name = normalizeLocalWebsiteName(req.body?.name ?? req.body?.url);
    ...
    const insertResult = await dbRunAsync(
      `
        INSERT INTO local_websites (name, url, country_id, created_by, updated_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `,
      [name, name, countryId, actorPlayerId, actorPlayerId]
    );
    ...
    if (String(error?.message || "").includes("UNIQUE")) {
      return res.status(409).json({ ok: false, message: "Local website with this name already exists" });
    }
  ...
  try {
    ...
    await dbRunAsync(
      `
        ...
        WHERE id = ?
      `,
      [name, name, countryId, actorPlayerId, localWebsiteId]
    );
...
function getManualChallengePlayerPeriodStatus(value) {
  ...
  return CHALLENGE_PLAYER_PERIOD_STATUSES.has(status) ? status : null;
}
...
async function resolveChallengeDuelCancelledByPlayerId(duel) {
  ...
  const matchedPlayerIds = participantIds.filter((playerId) => (
    (rows || []).some((row) => (
      normalizeNullableText(row?.player_1_id) === playerId
      || normalizeNullableText(row?.player_2_id) === playerId
    ))
  ));
  ...
  try {
    ...
    const matchRows = await dbAllAsync(
      `
        SELECT
          cr.id,
          COALESCE(cr.period_id, d.challenge_period_id) AS period_id,
          COALESCE(cr.player_1_id, d.player_1_id) AS player_1_id,
          COALESCE(cr.player_2_id, d.player_2_id) AS player_2_id,
        ...
        WHERE d.challenge_period_id = ?
          ...
          ${includeRemoved ? "" : "AND d.deleted_at IS NULL"}
          ${adminMode ? "" : "AND (d.player_1_id = ? OR d.player_2_id = ?)"}
        ORDER BY datetime(d.time_utc) DESC, d.id ASC
      `,
      adminMode ? [periodId] : [periodId, playerId, playerId]
    );
    const matchRowsByDuelId = new Map((matchRows || []).map((row) => [String(row?.duel_id || ""), row]));
    const matches = await attachGamesToChallengeRequests((matchRows || []).map(mapChallengeRequestWithPlayers).filter(Boolean));
    await Promise.all(matches.map(async (match) => {
      if (!match?.duel || match.duel.cancelled_by_player_id) return;
      const matchRow = matchRowsByDuelId.get(String(match.duel.id || ""));
      match.duel.cancelled_by_player_id = await resolveChallengeDuelCancelledByPlayerId({
        ...match.duel,
        challenge_period_id: match.period_id,
        player_1_id: match.player_1_id,
        player_2_id: match.player_2_id,
        updated_by: matchRow?.duel_updated_by,
      });
    }));
    if (adminMode) {
      return res.json({
...
app.patch("/challenge-periods/:id/requests/:requestId/cancel", requireAuthenticated, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const lockedCanCancelRequest = lockedRequest.created_by_player_id === playerId
        || lockedRequest.awaiting_player_id !== playerId;
      if (!lockedCanCancelRequest) {
        const error = new Error("Only the player waiting for a response can cancel this request");
        error.httpStatus = 403;
        throw error;
      }
      ...
      await dbRunAsync(
        `
          ...
          SET
            status = 'Cancelled',
            cancelled_by_player_id = ?,
            cancellation_reason = 'time_change_cancelled',
            cancelled_at = CURRENT_TIMESTAMP,
            updated_by = ?,
            updated_at = CURRENT_TIMESTAMP
          WHERE challenge_period_id = ?
            AND challenge_request_id = ?
            AND status IN ('Draft', 'Requested new time', 'Planned')
            AND source_type = 'challenge'
            AND deleted_at IS NULL
        `,
        [playerId, playerId, periodId, requestId]
      );
      await dbRunAsync("COMMIT");
    } catch (error) {
      await dbRunAsync("ROLLBACK").catch(() => {});
    ...
    if (beforeDuel && afterDuel?.status === "Cancelled" && beforeDuel.status !== "Cancelled") {
      logAuditEvent({
        ...getAuditActor(req.user),
        event_type: "challenge_duel.time_change_cancelled",
        entity_type: "duel",
        action: "update",
    ...
    if (error?.httpStatus) {
      return res.status(error.httpStatus).json({ ok: false, message: error.message || "Failed to cancel Challenge request" });
    }
...
app.patch("/challenge-periods/:id/requests/:requestId/decline", requireAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (["Draft", "Requested new time"].includes(beforeDuel?.status) && afterDuel?.status === "Cancelled") {
      logAuditEvent({
        ...getAuditActor(req.user),
...
app.patch("/challenge-periods/:id/player-status", requireAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await dbRunAsync("BEGIN IMMEDIATE TRANSACTION");
    try {
      if (shouldCloseChallengeRequestsForPlayerStatus(nextStatus)) {
        await dbRunAsync(
          `
            UPDATE duels
            SET
              status = 'Cancelled',
              cancelled_by_player_id = ?,
              cancellation_reason = 'player_closed_to_requests',
              cancelled_at = CURRENT_TIMESTAMP,
              updated_by = ?,
              updated_at = CURRENT_TIMESTAMP
            WHERE challenge_period_id = ?
          ...
          `,
          [playerId, playerId, periodId, periodId, playerId, playerId]
        );
        await dbRunAsync(
          `
            UPDATE challenge_requests
  ...
  db.all(
    `
      SELECT
        ...
        END AS access_via_access_users,
        (
          SELECT GROUP_CONCAT(tt_current.team_id, '|')
          FROM tournament_teams tt_current
          WHERE upper(trim(tt_current.tournament_id)) = upper(trim(t.id))
            AND trim(tt_current.captain_id) = trim(?)
        ) AS captain_team_ids_csv
      FROM tournaments t
      ${visibilitySql}
...
app.post("/tournaments", requireAdmin, async (req, res) => {
  ...
  if (tournamentType === TOURNAMENT_TYPES.TEAMS && requestedTeamType && !isValidTeamType(requestedTeamType)) {
    return res.status(400).json({ ok: false, message: "team_type must be National or Club" });
  }
...
app.post("/icons", requireAdmin, async (req, res) => {
  try {
    ...
    const localWebsiteId = category === "Local Website"
    ...
    if (category === "Local Website") {
      if (!localWebsiteId) {
        return res.status(400).json({ ok: false, message: "local_website_id is required for Local Website icon" });
      }
    ...
    const insertResult = await dbRunAsync(
      `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `,
      [name, iconLink, category, associationId, localWebsiteId, normalizedStreamerId || null, actorPlayerId, actorPlayerId]
    );
  ...
  try {
    ...
    await dbRunAsync(
      `
        ...
        WHERE id = ?
      `,
      [name, iconLink, category, associationId, localWebsiteId, normalizedStreamerId || null, actorPlayerId, iconId]
    );
  ...
  db.all(
    `
      ...
      ORDER BY
        ...
        lower(COALESCE(lw.name, lw.url, CAST(ne.local_website_id AS TEXT), '')) ASC,
        ...
        OR (
          ...
          AND EXISTS (
            ...
            WHERE upper(trim(tt.tournament_id)) = upper(trim(m.tournament_id))
              AND trim(tt.captain_id) = trim(?)
              AND (
                upper(trim(tt.team_id)) = upper(trim(COALESCE(m.team_1, '')))
                OR upper(trim(tt.team_id)) = upper(trim(COALESCE(m.team_2, '')))
              )
          )
        )
      )
  ...
  const selectMatchFields = `
      SELECT
        ...
        CASE
          ...
          ELSE 0
        END AS tournament_access_via_access_users,
        (
          SELECT GROUP_CONCAT(tt_current.team_id, '|')
          FROM tournament_teams tt_current
          WHERE upper(trim(tt_current.tournament_id)) = upper(trim(m.tournament_id))
            AND trim(tt_current.captain_id) = trim(?)
        ) AS tournament_captain_team_ids_csv
      FROM matches m
    `;
        ...
        return canClosedTournamentCaptainAccessMatch(
          {
            ...
            access_role: tournamentAccessRole,
            access_via_access_users: Number(row?.tournament_access_via_access_users) === 1,
            captain_team_ids: normalizeTournamentCaptainTeamIds(row?.tournament_captain_team_ids_csv),
          },
          userAssociation,
          row?.team_1,
          row?.team_2
...
app.get("/public/team-official-matches", async (req, res, next) => {
  ...
  const resultsPageSize = Number.isInteger(rawResultsPageSize) && rawResultsPageSize > 0
    ...
    : null;
  const resultsPage = Number.isInteger(rawResultsPage) && rawResultsPage > 0
    ? rawResultsPage
    : 1;
  const resultsBeforeValue = String(req?.query?.results_before || "").trim();
  ...
  const matchFilterClauses = [
    ...
    "lower(trim(COALESCE(NULLIF(trim(t.subtype), ''), NULLIF(trim(t.access_type), ''), 'Friendly'))) IN ('2', 'closed', 'official')",
  ];
  ...
  const whereSql = matchFilterClauses.join("\n          AND ");
  const selectMatchFields = `
    SELECT
      m.id,
      m.tournament_id,
      ...
      t.logo AS tournament_logo,
      t.link AS tournament_link,
      COALESCE(NULLIF(trim(t.tournament_type), ''), 'Teams') AS tournament_type
    FROM matches m
    JOIN tournaments t
      ON upper(trim(COALESCE(t.id, ''))) = upper(trim(COALESCE(m.tournament_id, '')))
    LEFT JOIN teams team1
  ...
  try {
    const { rows: matchRows, pagination } = await loadMatchRows();
    const normalizedMatchIds = Array.from(new Set(
      (matchRows || [])
        .map((row) => String(row?.id || "").trim())
        .filter(Boolean)
    ...
    if (normalizedMatchIds.length) {
      ...
      if (normalizedDuelIds.length) {
        gameRows = await new Promise((resolve, reject) => {
          loadGamesByDuelIds(normalizedDuelIds, (gamesErr, rows) => {
            if (gamesErr) {
              reject(gamesErr);
              return;
            }
...
app.get("/public/friendly-matches", (req, res, next) => {
  ...
  return db.get(
    ...
    (tournamentErr, tournamentRow) => {
              ...
              (countErr, countRow) => {
                ...
                return db.all(
                  ...
                  (resultsErr, resultsMatchRows) => {
                    ...
                    return callback(
                      null,
                      [...(activeMatchRows || []), ...(resultsMatchRows || [])],
                      {
                        results: {
                          before: resultsBeforeIso,
                          page: safePage,
      ...
      return loadMatchRows((matchesErr, matchRows, pagination) => {
          if (matchesErr) return next(matchesErr);

...
async function loadImageByIdentifier(identifier, { includeDeleted = false } = {}) {
  ...
  const whereClause = Number.isInteger(numericId) && numericId > 0
    ? "i.id = ?"
    : "i.uuid = ?";
  const row = await dbGetAsync(
    `
      ...
      LIMIT 1
    `,
    [Number.isInteger(numericId) && numericId > 0 ? numericId : raw]
  );
  ...
  `, (createErr) => {
    ...
    db.all("PRAGMA table_info(images)", (pragmaErr, columns) => {
      ...
      db.run(
        ...
        (normalizeErr) => {
          if (normalizeErr) {
            console.error("Failed to normalize images schema data", normalizeErr);
          }
        }
      );
...
app.get("/public/images", async (req, res) => {
  try {
    ...
    return res.json({ ok: true, images: (rows || []).map(normalizeImageRow) });
  } catch (error) {
...
app.get("/images", requireAuthenticated, async (req, res) => {
  try {
    ...
    if (!isAdmin) {
      ...
      const ownerUserId = normalizePositiveInteger(req.query.owner_user_id);
      if (!ownerUserId) {
...
app.get("/images/:id", requireAuthenticated, async (req, res) => {
  try {
    const image = await loadImageByIdentifier(req.params.id, {
...
app.post("/images", requireAuthenticated, async (req, res) => {
  try {
    ...
    const ownerUserId = isAdmin
      ? (normalizePositiveInteger(req.body?.owner_user_id ?? req.body?.ownerUserId) || Number(req.user.id))
      : Number(req.user.id);
    const uploadedByUserId = Number(req.user.id);
    const originalFilename = normalizeNullableText(req.body?.original_filename ?? req.body?.originalFilename);
    const storagePath = normalizeNullableText(req.body?.storage_path ?? req.body?.storagePath);
    const mimeType = normalizeNullableText(req.body?.mime_type ?? req.body?.mimeType);
    const fileSizeBytes = normalizeImageDimension(req.body?.file_size_bytes ?? req.body?.fileSizeBytes, "file_size_bytes");
    const width = normalizeImageDimension(req.body?.width, "width");
    const height = normalizeImageDimension(req.body?.height, "height");
    const title = normalizeNullableText(req.body?.title);
    const altText = normalizeNullableText(req.body?.alt_text ?? req.body?.altText);
    const caption = normalizeNullableText(req.body?.caption);
    ...
    const metadataJson = normalizeJsonText(req.body?.metadata_json ?? req.body?.metadataJson, "metadata_json", "{}");
    ...
    if (!storagePath) {
      return res.status(400).json({ ok: false, message: "storage_path is required" });
    }
    if (!mimeType) {
      return res.status(400).json({ ok: false, message: "mime_type is required" });
    }
    ...
    if (ownerUserId) {
      const ownerRow = await dbGetAsync("SELECT id FROM users WHERE id = ? LIMIT 1", [ownerUserId]);
      if (!ownerRow) {
    ...
    const createdRow = await loadImageByIdentifier(insertResult?.lastID);
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.patch("/images/:id", requireAuthenticated, async (req, res) => {
  try {
    ...
    if (!canManageImage(req.user, image)) {
      return res.status(403).json({ ok: false, message: "Forbidden" });
    }
    ...
    const ownerUserId = isAdmin && hasPayloadField(req.body, "owner_user_id", "ownerUserId")
      ? normalizePositiveInteger(req.body?.owner_user_id ?? req.body?.ownerUserId)
      : Number(image.owner_user_id);
    const originalFilename = hasPayloadField(req.body, "original_filename", "originalFilename")
      ? normalizeNullableText(req.body?.original_filename ?? req.body?.originalFilename)
      : image.original_filename;
    const storagePath = hasPayloadField(req.body, "storage_path", "storagePath")
      ? normalizeNullableText(req.body?.storage_path ?? req.body?.storagePath)
      : image.storage_path;
    const mimeType = hasPayloadField(req.body, "mime_type", "mimeType")
      ? normalizeNullableText(req.body?.mime_type ?? req.body?.mimeType)
      : image.mime_type;
    const fileSizeBytes = hasPayloadField(req.body, "file_size_bytes", "fileSizeBytes")
      ? normalizeImageDimension(req.body?.file_size_bytes ?? req.body?.fileSizeBytes, "file_size_bytes")
      : image.file_size_bytes;
    const width = hasPayloadField(req.body, "width")
      ? normalizeImageDimension(req.body?.width, "width")
      : image.width;
    const height = hasPayloadField(req.body, "height")
      ? normalizeImageDimension(req.body?.height, "height")
      : image.height;
    ...
    const altText = hasPayloadField(req.body, "alt_text", "altText")
      ? normalizeNullableText(req.body?.alt_text ?? req.body?.altText)
      : image.alt_text;
    const caption = hasPayloadField(req.body, "caption")
      ? normalizeNullableText(req.body?.caption)
      : image.caption;
    ...
    const metadataJson = hasPayloadField(req.body, "metadata_json", "metadataJson")
      ? normalizeJsonText(req.body?.metadata_json ?? req.body?.metadataJson, "metadata_json", "{}")
      : safeStringifyJson(image.metadata_json || {});
    ...
    const updatedRow = await loadImageByIdentifier(image.id, { includeDeleted: true });
    const changes = buildAuditChanges(image, updatedRow, IMAGE_AUDIT_FIELDS);
    if (!Object.keys(changes).length) {
      return res.json({ ok: true, image: updatedRow || null });
    }
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.delete("/images/:id", requireAuthenticated, async (req, res) => {
  try {
    ...
    const deletedRow = await loadImageByIdentifier(image.id, { includeDeleted: true });
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.get("/images/:id/imageables", requireAuthenticated, async (req, res) => {
  try {
    ...
    return res.json({ ok: true, imageables: (rows || []).map(normalizeImageableRow) });
  } catch (error) {
...
app.post("/imageables", requireAuthenticated, async (req, res) => {
  try {
    const imageId = normalizePositiveInteger(req.body?.image_id ?? req.body?.imageId);
    const imageableType = normalizeNullableText(req.body?.imageable_type ?? req.body?.imageableType);
    const imageableId = normalizeNullableText(req.body?.imageable_id ?? req.body?.imageableId);
    const role = normalizeNullableText(req.body?.role);
    const sortOrder = normalizeIntegerOrNull(req.body?.sort_order ?? req.body?.sortOrder) ?? 0;
    const cropJson = normalizeJsonText(req.body?.crop_json ?? req.body?.cropJson, "crop_json", null);

    if (!imageId) {
      return res.status(400).json({ ok: false, message: "image_id is required" });
    }
    if (!imageableType) {
      return res.status(400).json({ ok: false, message: "imageable_type is required" });
    }
    if (!imageableId) {
      return res.status(400).json({ ok: false, message: "imageable_id is required" });
    }

    const image = await loadImageByIdentifier(imageId);
    if (!image) {
      return res.status(400).json({ ok: false, message: "image_id must reference an existing image" });
    }
    if (!canManageImage(req.user, image)) {
      return res.status(403).json({ ok: false, message: "Forbidden" });
    }
    ...
    const createdRow = await loadImageableById(insertResult?.lastID);
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.get("/imageables/:id", requireAuthenticated, async (req, res) => {
  try {
    const imageable = await loadImageableById(req.params.id);
    if (!imageable) {
      return res.status(404).json({ ok: false, message: "Imageable not found" });
    }
    const image = await loadImageByIdentifier(imageable.image_id, {
      includeDeleted: Number(req.user?.admin) === 1,
    });
    if (!canAccessImage(req.user, image)) {
      return res.status(403).json({ ok: false, message: "Forbidden" });
...
app.patch("/imageables/:id", requireAuthenticated, async (req, res) => {
  try {
    const imageableIdParam = normalizePositiveInteger(req.params.id);
    const beforeRow = await loadImageableById(imageableIdParam);
    if (!beforeRow) {
      return res.status(404).json({ ok: false, message: "Imageable not found" });
    }
    const image = await loadImageByIdentifier(beforeRow.image_id, { includeDeleted: Number(req.user?.admin) === 1 });
    if (!canManageImage(req.user, image)) {
      return res.status(403).json({ ok: false, message: "Forbidden" });
    }

    const payloadId = normalizePositiveInteger(req.body?.id);
    const payloadImageId = normalizePositiveInteger(req.body?.image_id ?? req.body?.imageId);
    ...
    const imageableType = hasPayloadField(req.body, "imageable_type", "imageableType")
      ? normalizeNullableText(req.body?.imageable_type ?? req.body?.imageableType)
      : beforeRow.imageable_type;
    const imageableId = hasPayloadField(req.body, "imageable_id", "imageableId")
      ? normalizeNullableText(req.body?.imageable_id ?? req.body?.imageableId)
    ...
    const sortOrder = hasPayloadField(req.body, "sort_order", "sortOrder")
      ? (normalizeIntegerOrNull(req.body?.sort_order ?? req.body?.sortOrder) ?? 0)
      : beforeRow.sort_order;
    const cropJson = hasPayloadField(req.body, "crop_json", "cropJson")
      ? normalizeJsonText(req.body?.crop_json ?? req.body?.cropJson, "crop_json", null)
      : (beforeRow.crop_json === null ? null : safeStringifyJson(beforeRow.crop_json));
    if (!imageableType) {
      return res.status(400).json({ ok: false, message: "imageable_type is required" });
    }
    if (!imageableId) {
    ...
    const updatedRow = await loadImageableById(beforeRow.id);
    const changes = buildAuditChanges(beforeRow, updatedRow, IMAGEABLE_AUDIT_FIELDS);
    if (!Object.keys(changes).length) {
      return res.json({ ok: true, imageable: updatedRow || null });
    }
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.delete("/imageables/:id", requireAuthenticated, async (req, res) => {
  try {
    const beforeRow = await loadImageableById(req.params.id);
    if (!beforeRow) {
      return res.status(404).json({ ok: false, message: "Imageable not found" });
    }
    const image = await loadImageByIdentifier(beforeRow.image_id, { includeDeleted: Number(req.user?.admin) === 1 });
    ...
    await dbRunAsync("DELETE FROM imageables WHERE id = ?", [beforeRow.id]);
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.get("/imageables", requireAuthenticated, async (req, res) => {
  try {
    const imageableType = normalizeNullableText(req.query?.imageable_type ?? req.query?.imageableType);
    const imageableId = normalizeNullableText(req.query?.imageable_id ?? req.query?.imageableId);
...
app.post("/images/:id/variants", requireAuthenticated, async (req, res) => {
  try {
    ...
    const variant = normalizeNullableText(req.body?.variant);
    const storagePath = normalizeNullableText(req.body?.storage_path ?? req.body?.storagePath);
    const publicUrl = normalizeNullableText(req.body?.public_url ?? req.body?.publicUrl);
    const mimeType = normalizeNullableText(req.body?.mime_type ?? req.body?.mimeType);
    const fileSizeBytes = normalizeImageDimension(req.body?.file_size_bytes ?? req.body?.fileSizeBytes, "file_size_bytes");
    const width = normalizeImageDimension(req.body?.width, "width");
    const height = normalizeImageDimension(req.body?.height, "height");
    ...
    if (!variant) {
      return res.status(400).json({ ok: false, message: "variant is required" });
    }
    if (!storagePath) {
      return res.status(400).json({ ok: false, message: "storage_path is required" });
    }
    ...
    const createdRow = await loadImageVariantById(insertResult?.lastID);
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.get("/image-variants/:id", requireAuthenticated, async (req, res) => {
  try {
    const variant = await loadImageVariantById(req.params.id);
    if (!variant) {
      return res.status(404).json({ ok: false, message: "Image variant not found" });
    }
    const image = await loadImageByIdentifier(variant.image_id, {
      includeDeleted: Number(req.user?.admin) === 1,
    });
    if (!canAccessImage(req.user, image)) {
      return res.status(403).json({ ok: false, message: "Forbidden" });
...
app.patch("/image-variants/:id", requireAuthenticated, async (req, res) => {
  try {
    const beforeRow = await loadImageVariantById(req.params.id);
    if (!beforeRow) {
      return res.status(404).json({ ok: false, message: "Image variant not found" });
    }
    const image = await loadImageByIdentifier(beforeRow.image_id, { includeDeleted: Number(req.user?.admin) === 1 });
    ...
    const variant = hasPayloadField(req.body, "variant")
      ? normalizeNullableText(req.body?.variant)
      : beforeRow.variant;
    const storagePath = hasPayloadField(req.body, "storage_path", "storagePath")
      ? normalizeNullableText(req.body?.storage_path ?? req.body?.storagePath)
      : beforeRow.storage_path;
    const publicUrl = hasPayloadField(req.body, "public_url", "publicUrl")
      ? normalizeNullableText(req.body?.public_url ?? req.body?.publicUrl)
    ...
    const updatedRow = await loadImageVariantById(beforeRow.id);
    const changes = buildAuditChanges(beforeRow, updatedRow, IMAGE_VARIANT_AUDIT_FIELDS);
    if (!Object.keys(changes).length) {
      return res.json({ ok: true, variant: updatedRow || null });
    }
    return logAuditEvent(
      {
        ...getAuditActor(req.user),
...
app.delete("/image-variants/:id", requireAuthenticated, async (req, res) => {
  try {
    ...
    await dbRunAsync("DELETE FROM image_variants WHERE id = ?", [beforeRow.id]);
    return logAuditEvent(
      {
        ...getAuditActor(req.user),

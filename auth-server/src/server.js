...
function mapChallengePeriodForPlayer(row) {
  ...
  if (duelStatus === "Done") {
    playerStatus = "played";
  } else if (row?.challenge_duel_id && ["Draft", "Requested new time", "Planned", "In progress", "Error"].includes(duelStatus)) {
          ...
          return db.serialize(() => {
            ...
            db.run(
              `
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
              `,
              [id, name, shortTitle, logo, link, tournamentType, teamType, category, subtype, accessType, playerHubVisibility, lineupSizeType, lineupSize],
              (insertErr) => {
                if (insertErr) {
                  db.run("ROLLBACK");
                  if (String(insertErr.message || "").includes("UNIQUE")) {
